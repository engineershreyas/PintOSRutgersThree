...
/* Returns a PTE that points to PAGE.
   ...
   If can_write is true then it will be can_write as well.
   The page will be usable only by ring 0 code (the kernel). */
static inline uint32_t pte_create_kernel (void *page, bool can_write) {
  ASSERT (pg_ofs (page) == 0);
  return vtop (page) | PTE_P | (can_write ? PTE_W : 0);
}
...
static inline uint32_t pte_create_user (void *page, bool can_write) {
  return pte_create_kernel (page, can_write) | PTE_U;
}
