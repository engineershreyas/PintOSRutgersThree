...
size_t swap_out (void *frame){
  if(!swap_block || !swap_map) PANIC("Swap partition needed but not present");
  ...
  size_t i;
  for(i = 0; i < SECS_PER_PAGE; i++){
    block_write(swap_block, free_ind * SECS_PER_PAGE + i, (uint8_t *) frame + i * BLOCK_SECTOR_SIZE);
  }
...
void swap_in (size_t used_index, void *frame){
  ...
  size_t i;
  for(i = 0; i < SECS_PER_PAGE; i++){
    block_read(swap_block, used_index * SECS_PER_PAGE + i, (uint8_t) frame + i * BLOCK_SECTOR_SIZE);
  }
